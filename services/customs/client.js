...
if (!window.SwVersion) {
    var onloadFunc = function () {
        ...
        installlerUrl = "http://update.singlewindow.cn/downloads/EportClientSetup_V1.5.14.exe";
        if (!installlerUrl) {
            setTimeout(function () {
                installlerUrl = "http://update.singlewindow.cn/downloads/EportClientSetup_V1.5.14.exe";
 ...
 * @returns {*}
 */
var getCliectReadyState = function (thisWs) {
    var currWs = thisWs;
    if (!currWs) {
        return 0;
    }
...
var conn = function () {
    var state = getCliectReadyState(ws);
    //readyState->0:尚未建立连接;1:已经建立连接;2:正在关闭;3:已经关闭或不可用
    //=0的时候不要创建新的了,连接慢的情况下,创建新的可能会更慢!
    if (!ws || state === 2 || state === 3) {
        try {
            ...
            //var websocketurl = "ws://10.88.205.222:61232";
            //var websocketurl = "ws://127.0.0.1:61232";
            if (window.console && window.console.log) {
                window.console.log("%c 使用" + websocketurl + "连接控件服务", 'color:#1941EC;font-size:12px');
            }
            ws = new window.WebSocket(websocketurl); //'wss://testlocal.sino-clink.com.cn:61231'
            //WebSocket事件
    ...
    if (getCliectReadyState(ws) === 1) {
        ...
        var waitForWebSocketConn = function () {
            if (times > 9) {
                ...
                if (getCliectReadyState(ws) === 0) {//试图等待500毫秒
                    setTimeout(
                        function () {
                            if (getCliectReadyState(ws) === 1) {
                                ws.send(msg);
                ...
                } else if (getCliectReadyState(ws) === 2 || getCliectReadyState(ws) === 3) {//试图重连
                    times++;
                    conn();//重连
                }
            }
...
var getWebSocketReadyState = function (thisWs) {
    ...
    return getCliectReadyState(currWs);
};
...
var EportClient = {
    ...
    isInstalledTest: function (func, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8, arg9, arg10) {//这里argX只多不少即可
        ...
        window.EportClient.isInstalled(DefaultType, function (msg) {
            if (msg.Result) {
